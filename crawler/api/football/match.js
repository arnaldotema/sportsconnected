...
const {
  updateCompetitionStandingsAndStats
} = require("../../services/football/competitionSeason");
const { updateByZeroZeroId } = require("../../services/football/match");
const {
  getMatchTeamsByZeroZeroId,
  ...
  for (let i = 0; i < eventsLabels.length; i++) {
    ...
    switch (label.html()) {
      ...
      case "R":
        ...
        break;
      case "S":
        playerStats.yellow_cards = playerStats.yellow_cards.concat(
          _getEventsTimes(times)
        );
...
const initializeMatchModel = function(match, res, done, cb) {
  //Competition
  ...
  match.duration = res.$(".extratime").length === 0 ? 90 : 120;

  // Teams

  let homeZeroZeroId =
    res.$("#page_header .info .content .home .team a").length > 0
      ? res
          .$("#page_header .info .content .home .team a")
          .attr("href")
          .match(/\d+/g)[0]
      : 0;

  let awayZeroZeroId =
    res.$("#page_header .info .content .away .team a").length > 0
      ? res
          .$("#page_header .info .content .away .team a")
          .attr("href")
          .match(/\d+/g)[0]
      : 0;
...
exports.processMatchIds = function(match, res, done, cb) {
  ...
  if (report.length === 0) {
    logger.error(
      "Match with zerozero team_id: " +
        res.options.zerozeroId +
        " has no report"
    );
    //proxyFailCallback(res, done);
  } else {
    res
      .$(report[0])
      .find(".player")
      .each(function() {
        _processPlayerEvents(
          ...
          this,
          true,
          homeTeamIds.main_lineup,
          match.home_team.main_lineup
        );
      });
    res
      .$(report[1])
      ...
      .each(function() {
        _processPlayerEvents(
          ...
          this,
          true,
          awayTeamIds.main_lineup,
          match.away_team.main_lineup
        );
      });
    res
      .$(report[2])
      ...
      .each(function() {
        _processPlayerEvents(
          ...
          this,
          false,
          homeTeamIds.reserves,
          match.home_team.reserves
        );
      });
    res
      .$(report[3])
      ...
      .each(function() {
        _processPlayerEvents(
          ...
          this,
          false,
          awayTeamIds.reserves,
          match.away_team.reserves
        );
      });
    res
      .$(report[4])
      .find(".player .name .text a")
      .each(function() {
        homeTeamIds.staff.push(
          +res
            .$(this)
            .attr("href")
            .match(/\d+/g)[0]
    ...
    function(err, result) {
      if (err) {
        ...
        result[0].home_team.forEach(function(value, index) {
          ...
          match.home_team.main_lineup[index].avatar =
            value.personal_info.avatar;
          match.home_team.main_lineup[index].positions =
            value.personal_info.positions;
          match.home_team.main_lineup[index].nationality =
        ...
        result[0].away_team.forEach(function(value, index) {
          ...
          match.away_team.main_lineup[index].avatar =
            value.personal_info.avatar;
          match.away_team.main_lineup[index].positions =
            value.personal_info.positions;
          match.away_team.main_lineup[index].nationality =
            value.personal_info.nationality;
        ...
        result[0].home_team_reserves.forEach(function(value, index) {
          ...
          match.home_team.reserves[index].positions =
            value.personal_info.positions;
          match.home_team.reserves[index].nationality =
            value.personal_info.nationality;
        ...
        result[0].away_team_reserves.forEach(function(value, index) {
          ...
          match.away_team.reserves[index].positions =
            value.personal_info.positions;
          match.away_team.reserves[index].nationality =
            value.personal_info.nationality;
          match.away_team.reserves[index].id = value._id;
          match.away_team.reserves[index].user_info_id = value.user_info_id;
        });

        updateByZeroZeroId(res.options.zerozeroId, match, function(
          err,
          result
        ) {
          if (err) {
            ...
            const embedMatch = {
              ...
              competition_season: {
                ...
                competition_id:
                  res.options.match.competition_season.competition_id,
...
exports.processMatchPlayers = function(nestedMatch, res, done, cb) {
  updateUserInfosStats(res.options.match, nestedMatch, function(err, result) {
    if (err) {
      logger.error(err);
      failBack(res, done);
    } else {
...
exports.processMatchTeams = function(nestedMatch, res, done, cb) {
  updateTeamsStandings(res.options.match, nestedMatch, function(err, result) {
    if (err) {
      logger.error(err);
      failBack(res, done);
    } else {
...
exports.processMatchCompetition = function(nestedMatch, res, done, cb) {
  updateCompetitionStandingsAndStats(res.options.match, nestedMatch, function(
    err,
    result
  ) {
    if (err) {
...
exports.processUserInfoRegex = function(res, done) {
  ...
  getMatchUserInfos(homeTeamIds, awayTeamIds, function(err, result) {
    if (err) {
      logger.error(err);
      failBack(res, done);
    } else {
      ...
      result[0].home_team.forEach(function(value, index) {
        regexes[value._id] = _updateRegex(
          value.actions_regex,
          res.options.match.home_team.main_lineup[index]
        );
      });
      ...
      result[0].away_team.forEach(function(value, index) {
        regexes[value._id] = _updateRegex(
          value.actions_regex,
          res.options.match.away_team.main_lineup[index]
        );
      });
      ...
      result[0].home_team_reserves.forEach(function(value, index) {
        regexes[value._id] = _updateRegex(
          value.actions_regex,
          res.options.match.home_team.reserves[index]
        );
      });
      ...
      result[0].away_team_reserves.forEach(function(value, index) {
        regexes[value._id] = _updateRegex(
          value.actions_regex,
          res.options.match.away_team.reserves[index]
        );
      });

      updateRegexNewMatch(regexes, function(err, result) {
        if (err) {
          logger.error(err);
          failBack(res, done);
        } else {
...
exports.processMatchInfo = function(err, res, done) {
  ...
  initializeMatchModel(match, res, done, function(match, res, done) {
    processMatchIds(match, res, done, function(nestedMatch, res, done) {
      processMatchCompetition(nestedMatch, res, done, function(
        nestedMatch,
        res,
        done
      ) {
        processMatchTeams(nestedMatch, res, done, function(
          nestedMatch,
          res,
          done
        ) {
