...
const {
  updateByZeroZeroId: updateCompetitionByZeroZeroId
} = require("../../services/football/competition");
const {
  updateByZeroZeroId: updateCompetitionSeasonByZeroZeroId
} = require("../../services/football/competitionSeason");
const { insertFutureMatches } = require("../../services/football/match");
const { getSeasonByName } = require("../../services/football/season");
    ...
    getSeasonByName(res.options.season_name, function(err, result) {
      if (err) {
        ...
        competition_season.name =
          res.$("#page_header .factsheet .name").length > 0
            ? res.$("#page_header .factsheet .name").html()
            : "";

        competition_season.avatar =
          res.$("#page_header_container .logo img").length > 0
            ? "http://www.zerozero.pt/" +
              res.$("#page_header_container .logo img")[0].attribs["data-cfsrc"]
            : "";

        updateCompetitionSeasonByZeroZeroId(
...
const updateCompetitionSeasonMatches = async (err, res, done) => {
  ...
  res.$("#team_games tbody .parent").each(() => {
    const matchDate = new Date(
      res.$(this.children[1]).html() + " " + res.$(this.children[2]).html()
    );
    ...
    (err, result) => {
      if (err) {
        ...
        editionIds.forEach(function(edition) {
          queue({
            uri: format(baseUris.COMPETITION_EDITION, {
