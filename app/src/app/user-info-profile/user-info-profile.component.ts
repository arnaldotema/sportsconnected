...
export class UserInfoProfileComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    this.userInfoService.getUserInfo(this.id).subscribe(userInfo => {
      // Check if user is already being followed by the session user

      if (
        userInfo.followers.indexOf(
          this.authenticationService.getSessionUser().profile_id
        ) > -1
      ) {
      ...
      if (
        this.authenticationService.getSessionUser().profile_id == userInfo._id
      ) {
      ...
      let birth_date = new Date(
        userInfo.current_season.personal_info.date_of_birth
      );
      let ageDifMs = Date.now() - birth_date.getTime();
      let ageDate = new Date(ageDifMs);
      userInfo.current_season.personal_info.age = Math.abs(
      ...
      userInfo.current_season.media = [
        {
          title:
            userInfo.current_season.personal_info.name +
            " marca Hat-trick em jogo decisivo",
          author: "A Bola.",
          date: "08-05-2018",
          image:
      ...
      if (
        ...
        !userInfo.recommendations.top_5 ||
        userInfo.recommendations.top_5.length < 1
      ) {
        userInfo.recommendations = {
          list: [1, 2, 3],
          top_5: [
      ...
      userInfo.current_season.personal_info.avatar =
        userInfo.current_season.personal_info.avatar +
        "?random=" +
        _make_random_text();
  ...
  loadChart() {
    ...
    this.options = {
      title: {
        text:
          'Vota nas "Skills" de ' +
          this.viewModel.current_season.personal_info.name,
        display: true
      },
      legend: {
        display: false
  ...
  addRecommendation(): void {
    ...
    dialogRef.afterClosed().subscribe(rec_object => {
      ...
      this.userInfoService
        .createRecommendation(recommendation, this.viewModel._id)
        .subscribe(done => {
          if (done) {
  ...
  voteSkill(event): void {
    ...
    this.userInfoService
      .voteForSkill(skillName, this.session_user.profile_id, this.viewModel._id)
      .subscribe(user => {
        if (user) {
          this.labels = [];
          this.labels_values = [];
      ...
      .subscribe(done => {
        if (done) {
          this.labels.forEach((label, key) => {
            if (label == skillName) ++this.labels_values[key];
            this.cdRef.detectChanges();
            this._chart.update();
          });
          this.toastr.success("Voto registado!");
  ...
  follow() {
    this.userInfoService.follow(this.viewModel._id).subscribe(done => {
      if (done) {
        this.isFollowing = true;
        this.cdRef.detectChanges();
      }
  ...
  unfollow() {
    this.userInfoService.unfollow(this.viewModel._id).subscribe(done => {
      if (done) {
