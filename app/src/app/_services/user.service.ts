...
export class UserService {
  mockUser: User = {
    ...
    name: "Arnaldo Tema",
    avatar:
      "https://images.unsplash.com/photo-1529808431514-65ce4a3d59f2?ixlib=rb-0.3.5&ixid=eyJhcHBfaWQiOjEyMDd9&s=eb011da7b9ee20720f6922750c7ce6c9&auto=format&fit=crop&w=676&q=80",
    last_login: "",
  ...
  getSessionUser(): Observable<User> {
    ...
    return this.http.get<User>("/api/auth/" + id, this.requestOptions).pipe(
      tap(data => {
  ...
  aggregate_profile(profile_id): Observable<SessionUser> {
    ...
    console.log(
      "Authentication Service Current Token: " +
        this.authenticationService.token
    );

    return this.http
      .post<SessionUser>(
        "/api/auth/" + id + "/aggregate-profile",
        body,
        this.requestOptions
      )
      .pipe(
