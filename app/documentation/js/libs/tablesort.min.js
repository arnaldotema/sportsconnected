      ...
      return (
        window.CustomEvent && 'function' == typeof window.CustomEvent
          ...
          : ((b = document.createEvent('CustomEvent')),
            b.initCustomEvent(a, !1, !1, void 0)),
        b
      )
    },
    d = function(a) {
    ...
    (a.prototype = {
        ...
        if (
          ...
          if (a.tHead && a.tHead.rows.length > 0) {
            for (e = 0; e < a.tHead.rows.length; e++)
              if (
                'thead' === a.tHead.rows[e].getAttribute('data-sort-method')
              ) {
                c = a.tHead.rows[e]
                break
              }
        ...
        if (c) {
          ...
          for (e = 0; e < c.cells.length; e++)
            (f = c.cells[e]),
              ...
              'none' !== f.getAttribute('data-sort-method') &&
                ((f.tabindex = 0),
                f.addEventListener('click', h, !1),
                null !== f.getAttribute('data-sort-default') && (d = f))
      ...
      sortTable: function(a, g) {
        var h = this,
          ...
          l = [],
          m = h.thead ? 0 : 1,
          n = a.getAttribute('data-sort-method'),
          o = a.getAttribute('aria-sort')
        if (
          (h.table.dispatchEvent(c('beforeSort')),
          g ||
            ((o =
              'ascending' === o
                ? 'descending'
                : 'descending' === o
                ? 'ascending'
                : h.options.descending
                ? 'descending'
                : 'ascending'),
            a.setAttribute('aria-sort', o)),
          !(h.table.rows.length < 2))
        ) {
          if (!n) {
            for (; l.length < 3 && m < h.table.tBodies[0].rows.length; )
              (k = d(h.table.tBodies[0].rows[m].cells[i])),
                (k = k.trim()),
                k.length > 0 && l.push(k),
                m++
            if (!l) return
          ...
          for (h.col = i, m = 0; m < h.table.tBodies.length; m++) {
            ...
            if (!(h.table.tBodies[m].rows.length < 2)) {
              for (p = 0; p < h.table.tBodies[m].rows.length; p++)
                (k = h.table.tBodies[m].rows[p]),
                  'none' === k.getAttribute('data-sort-method')
                    ? (r[s] = k)
                    : q.push({ tr: k, td: d(k.cells[h.col]), index: s }),
                  s++
              for (
                'descending' === o
                  ? q.sort(f(j, !0))
                  : (q.sort(f(j, !1)), q.reverse()),
                  p = 0;
                p < s;
                p++
              )
                r[p] ? ((k = r[p]), t++) : (k = q[p - t].tr),
                  h.table.tBodies[m].appendChild(k)
            }
          }
